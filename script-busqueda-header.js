// ==================== BÚSQUEDA EN HEADER ====================
const productosData = [
    { id: '1', nombre: 'Paneles Hexagonales LED', precio: 189.99, imagen: 'images/productos/Panel hexagonal.png', descripcion: 'Diseña tu espacio con nuestros paneles hexagonales LED. Perfectos para crear efectos de luz únicos en tu setup gaming o habitación.', caracteristicas: 'Iluminación RGB 16M colores|Control mediante app móvil|Soporte asistente de voz|Mounting adhesivo incluido|Diseño modular conectable' },
    { id: '2', nombre: 'Teclado Mecánico RGB', precio: 349.99, imagen: 'images/productos/Teclado RGB.png', descripcion: 'Teclado mecánico premium con iluminación RGB personalizable. Switches azul para una experiencia táctil excepcional.', caracteristicas: 'Switches mecánicos azul|Iluminación RGB por tecla|N-Key rollover anti-ghosting|Cable USB-C desmontable|Reposabrazos aluminio antideslizante' },
    { id: '3', nombre: 'Auriculares Gaming Pro', precio: 249.99, imagen: 'images/productos/Cascos.png', descripcion: 'Auriculares gaming con sonido envolvente 7.1, micrófono retráctil e iluminación LED.', caracteristicas: 'Sonido surround 7.1|Micrófono con cancelación de ruido|Iluminación LED RGB|Compatibles PC, PS4, Xbox|Cojines memory foam' },
    { id: '4', nombre: 'Lámpara LED de Diseño', precio: 249.99, imagen: 'Lámpara_de_diseño.png', descripcion: 'Lámpara de diseño minimalista con luz LED regulable. Perfecta para escritorios y espacios de trabajo.', caracteristicas: '3 modos temperatura color|Brillo ajustable 10-100%|Pantalla LED con hora|Base estable antideslizante|Consumo 12W' },
    { id: '5', nombre: 'Pack Completo Gaming', precio: 599.99, imagen: 'images/portada-2.png', descripcion: 'Pack completo para tu setup gaming: teclado, ratón, alfombrilla y auriculares con iluminación RGB sincronizada.', caracteristicas: 'Teclado mecánico RGB|Ratón 12000 DPI|Auriculares 7.1|Alfombrilla XL 90x40cm|Cable USB centralizado' },
    { id: '6', nombre: 'Rótulo LED 3D', precio: 479.99, imagen: 'images/portada-3.png', descripcion: 'Rótulo LED 3D personalizado con la frase que tú elijas. Ilumina tu mundo con estilo.', caracteristicas: 'Led de neón flexible|16 millones de colores|Mando a distancia incluido|Tamaño personalizable|Fácil instalación' },
    { id: '7', nombre: 'Panel RGB 60cm', precio: 219.99, imagen: 'images/portada-4.png', descripcion: 'Panel LED de grandes dimensiones para crear ambientes únicos. Control remoto incluido.', caracteristicas: '60cm de longitud|Control remoto 44 teclas|16 escenas predefinidas|Compatibilidad Alexa|Sincronización con música' }
];
...
function abrirPaginaProducto(imagen, nombre, precio, descripcion, caracteristicas) {
    ...
    document.getElementById('btnCarritoPagina').onclick = function () {
        if (typeof agregarAlCarrito === 'function') {
            ...
            const articulo = articulosCarrito ? articulosCarrito.find(a => a.id === id) : null;
            if (articulo) {
                // Ya existe, solo sumar la cantidad indicada
...
// ==================== FICHA DE PRODUCTO: INTERACTIVIDAD ====================

// Precio base guardado para cálculos de tallas
let precioBaseProductoActual = 0;

// Al abrir la ficha, inicializar precios de tallas y estado
function inicializarFichaDetalle(precioBase) {
    precioBaseProductoActual = precioBase;

    // Calcular precios de las tallas
    const extras = [0, 100, 300];
    document.querySelectorAll('.talla-opcion-detalle').forEach((btn, i) => {
        const precio = precioBase + extras[i];
        const span = btn.querySelector('.precio-base-talla');
        if (span) span.textContent = precio.toFixed(2);
    });

    // Activar primera talla
    const tallas = document.querySelectorAll('.talla-opcion-detalle');
    if (tallas.length > 0) {
        tallas.forEach(t => t.classList.remove('activo'));
        tallas[0].classList.add('activo');
    }

    // Reset favorito
    const favBtn = document.getElementById('btnFavoritoDetalle');
    if (favBtn) favBtn.classList.remove('activo');

    // Reset colores
    document.querySelectorAll('.color-dot-detalle').forEach((d, i) => {
        d.classList.toggle('activo', i === 0);
    });

    // Reset filtros reseñas
    document.querySelectorAll('.filtro-resena').forEach((f, i) => {
        f.classList.toggle('activo', i === 0);
    });

    // Bundle precio actualizado
    const bundlePrecio = document.getElementById('bundlePrecio');
    if (bundlePrecio) bundlePrecio.textContent = (precioBase + 59.99).toFixed(2) + '€';
}

function seleccionarTallaDetalle(btn) {
    document.querySelectorAll('.talla-opcion-detalle').forEach(t => t.classList.remove('activo'));
    btn.classList.add('activo');
    const extra = parseFloat(btn.dataset.precioExtra) || 0;
    const nuevoPrecio = precioBaseProductoActual + extra;
    const precioEl = document.getElementById('precioPaginaDetalle');
    if (precioEl) precioEl.textContent = nuevoPrecio.toFixed(2) + '€';
}

function seleccionarColorDetalle(btn) {
    document.querySelectorAll('.color-dot-detalle').forEach(d => d.classList.remove('activo'));
    btn.classList.add('activo');
}
...
function filtrarResenas(btn) {
    document.querySelectorAll('.filtro-resena').forEach(f => f.classList.remove('activo'));
    btn.classList.add('activo');
}

// Extender abrirPaginaProducto para inicializar la ficha completa
const _originalAbrirPaginaProducto = abrirPaginaProducto;
abrirPaginaProducto = function(imagen, nombre, precio, descripcion, caracteristicas) {
    _originalAbrirPaginaProducto(imagen, nombre, precio, descripcion, caracteristicas);
    inicializarFichaDetalle(parseFloat(precio));
};
